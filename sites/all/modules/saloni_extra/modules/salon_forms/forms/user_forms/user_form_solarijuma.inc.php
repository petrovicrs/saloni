...
function salon_forms_solarijuma() {
  $page = array();
  $conf = getConfigByType(TYPE_SOLARIJUM);
  $table = new SalonTimeSpanTable($conf['od'], $conf['do'], $conf['interval']);
  $date = getDateFromParameters();
  $dateString = $date->toString();
  $appointments = AppointmentEntity::create()->getCols(array('id', 'termin_od', 'termin_do', 'id_klijent'), ' datum_termina = \''.$dateString.'\' AND id_usluge=\'2\' ');
...
function salon_forms_solarijuma_zakazi() {
  $page = [];
  $conf = getConfigByType('solarijum');
  __add_zakazi_specific_interval_js($conf['od'], $conf['do'], $conf['interval']);
  $serviceGroup = ServicesGroupEntity::create()->getColWithIds('opis', ' tip_grupa_usluga = \'2\' ');
